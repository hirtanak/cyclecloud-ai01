...
bash 'Install cyclecloud python api' do
  code <<-EOH
    ...
    jetpack download --project ai01 #{node[:slurm][:cyclecloud_api]} #{node[:slurm][:cyclecloud_api]} || exit 1;
...
bash 'Install job_submit/cyclecloud' do
  code <<-EOH
    jetpack download --project ai01 job_submit_cyclecloud_#{node[:platform]}_#{slurmver}.so  /usr/lib64/slurm/job_submit_cyclecloud.so || exit 1;
